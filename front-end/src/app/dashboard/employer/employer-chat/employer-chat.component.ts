...
@Component({
  ...
  styleUrls: ['./employer-chat.component.scss'],
  providers: [Room, Notification, MessageObj]
})
export class EmployerChatComponent implements OnInit, OnDestroy, AfterViewChecked {
  ...
  ngOnInit(): void {
    ...
    this._roomChatService.userRoomLeftEvent().subscribe((room) => {
      let user = room.users.filter(ele => ele._id != this.userData.id)[0];
      this.notification.display(`${user.userName} has ended the chat.`);
      this.messageList = [];
      let temp = {
  ...
  getAllChatRooms(): void {
    this._roomService.getAllRoomByNamespace('EMPLOYER').subscribe(resp => {
